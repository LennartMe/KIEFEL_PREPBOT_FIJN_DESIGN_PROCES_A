--Vormgerelateerd Bewerkingstijden controle
...
SELECT 
	...
	FORMAT(CAST((ROUND(((TRY_CAST(REPLACE(MachineUrenDummy, ',', '.') AS FLOAT) / TRY_CAST(REPLACE(Cavities_Dummy, ',', '.') AS FLOAT)) * TRY_CAST(REPLACE(Cavities, ',', '.') AS FLOAT) + TRY_CAST(REPLACE(MachineSteltijdDummy, ',', '.') AS FLOAT)) * 4,0) / 4.0) as decimal(10,3)), 'N2', 'nl-NL')
...
(SELECT
    ...
    END as [MachineUrenNew],
		...
		FORMAT(CAST((d.MachSetupTime / 3600) as decimal(10,3)), 'N2', 'nl-NL') as [MachineSteltijdDummy],
	CASE 
...
WHERE MachineUrenOld <> MachineUrenDummy_Calculated
order by linenr