...
SELECT DISTINCT --ProdStatusCode,
    LineNr,
...
FROM (
    ...
    END AS [Uren Type],
		...
		WHEN pboo.MachCycleTime > 0 
			...
			ELSE (CASE WHEN pboo.MachGrpCode IN ('DSO','FR','VO','CAM') THEN FORMAT(ISNULL(CAST((ISNULL(nc.NCMachCycleTime, 0) / NULLIF(pboo.Qty, 0) / 3600.0) AS decimal(10,3)), 0), 'N2', 'nl-NL') 
																		ELSE FORMAT(ISNULL(CAST((ISNULL(nc.NCMachCycleTime, 0) / NULLIF(pboo.Qty, 0) * @QuantityNew / 3600.0) AS decimal(10,3)), 0), 'N2', 'nl-NL') END)
			END AS [MachineUren], 
		...
		WHEN CQRI.CavityBased = 1 
		THEN 
		FORMAT(ISNULL(CAST((((ISNULL(DUMMY.MachCycleTime, 0)/28) * CavitiesDecimal) * @QuantityNew / 3600.0) AS decimal(10,3)), 0), 'N2', 'nl-NL')
		ELSE 
		...
		WHEN pboo.OccupationCycleTime > 0
			...
			ELSE (CASE WHEN pboo.MachGrpCode IN ('DSO','FR','VO','CAM') THEN FORMAT(ISNULL(CAST((ISNULL(nc.NCOccupationCycleTime, 0) / NULLIF(pboo.Qty, 0) / 3600.0) AS decimal(10,3)), 0), 'N2', 'nl-NL') 
																		ELSE FORMAT(ISNULL(CAST((ISNULL(nc.NCOccupationCycleTime, 0) / NULLIF(pboo.Qty, 0) * @QuantityNew / 3600.0) AS decimal(10,3)), 0), 'N2', 'nl-NL') END)
			END AS [Manuren], 
		...
		WHEN CQRI.CavityBased = 1 
		THEN 
		FORMAT(ISNULL(CAST((((ISNULL(DUMMY.OccupationCycleTime, 0)/28) * CavitiesDecimal) * @QuantityNew / 3600.0) AS decimal(10,3)), 0), 'N2', 'nl-NL')
		ELSE 
	...
	OUTER APPLY (
    SELECT 
        SUM(t1.MachCycleTime) AS NCMachCycleTime,
        SUM(t1.OccupationCycleTime) AS NCOccupationCycleTime
    FROM T_LogHourProdBOO t1
    WHERE 
        t1.ProdHeaderDossierCode = pboo.ProdHeaderDossierCode
        AND t1.ProdBOOLineNr = pboo.ProdBOOLineNr
        AND NOT EXISTS (
            SELECT 1
            FROM T_ProcessedHourLines t4
            WHERE t4.ProcHourLineNr = t1.ProcHourLineNr
              AND t4.ProcHourLinesLineNr = t1.ProcHourLinesLineNr
              AND EXISTS (
                  SELECT 1 FROM T_ProcessedHour
                  WHERE ProcHourLineNr = t4.ProcHourLineNr AND PauseCalcType = 3
              )
        )
        AND NOT EXISTS (
            SELECT 1
            ...
            WHERE t4.ProcHourLineNr = t1.ProcHourLineNr
              ...
              AND EXISTS (
                  SELECT 1 FROM T_ProcessedHour
                  WHERE ProcHourLineNr = t4.ProcHourLineNr AND PauseCalcType = 1
              )
              AND EXISTS (
                  SELECT 1
                  FROM T_WorkingHourType
                  WHERE WorkingHourTypeCode = t4.WorkingHourTypeCode
                    AND WorkingHourTypeType = 1
              )
        )
) AS nc
